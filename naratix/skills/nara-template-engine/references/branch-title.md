...
4. Connected: offer to **try it on one product**, running the [test drive](test-drive.md) but ending in `generate-title` (`product_id`, the new `title_template_id`, the taxonomy). The new title lands in the product's title history.
...
`generate-seo` writes `meta_title` and/or `meta_description` for one product. It has no template — pass the language by English name and choose which fields to write (both default to true). The result goes to the product's SEO version history.
