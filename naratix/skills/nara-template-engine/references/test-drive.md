...
3. `generate-description` (`subject_type: product`, `subject_id`, template, taxonomy) returns a link and a `run_id` immediately. Hand the link over right away and say it fills itself in — the customer can watch it arrive rather than wait.
4. `list-runs` (`kind: generation`, that `run_id`) when they want to know it is done: `completed` comes with an excerpt of the text — report it ready; `failed` comes with the reason — say plainly that it failed and nothing on the product changed.
